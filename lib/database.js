...
module.exports = {
  ...
  get_txs: function(block, cb) {
    var txs = [];
    if (!block || !block.tx) return cb(txs);
  ...
  get_block_count: function(since, cb) {
    Tx.distinct('blockindex', { timestamp: { $gt: since } }).exec(function(err, data) {
      if (err) {
        console.log(err);
        return cb(null);
      } else {
    ...
    lib.syncLoop(
      ...
      function() {
        distribution.t_101plus.percent = parseFloat(
          100 - distribution.t_76_100.percent - distribution.t_51_75.percent - distribution.t_26_50.percent - distribution.t_1_25.percent
        ).toFixed(2);
        distribution.t_101plus.total = parseFloat(
          distribution.supply - distribution.t_76_100.total - distribution.t_51_75.total - distribution.t_26_50.total - distribution.t_1_25.total
        ).toFixed(8);
